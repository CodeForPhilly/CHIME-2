...
def __validate_file(reference_file_content, reference_file_header, reference_file_name):
    with open(reference_file_name, "r") as f:
        ...
        for i, row in enumerate(lines[1:]):
            ...
            assert (
                int(tokens[0]) == i
            ), f"Failed for file {reference_file_name} row {i+1}."
            assert (
                int(tokens[1]) == i - 2
            ), f"Failed for file {reference_file_name} row {i+1}."
            assert tokens[2] == row_date.strftime(
                "%Y-%m-%d"
            ), f"Failed for file {reference_file_name} row {i+1}."
            print(f"'{tokens[3]},{tokens[4]},{tokens[5]}'")
            assert not tokens[3].strip() or float(tokens[3]) == pytest.approx(
                reference_row[0], 1e-10, 1e-1
            ), f"Failed for file {reference_file_name} row {i+1}."
            assert not tokens[4].strip() or float(tokens[4]) == pytest.approx(
                reference_row[1], 1e-10, 1e-2
            ), f"Failed for file {reference_file_name} row {i+1}."
            assert not tokens[5].strip() or float(tokens[5]) == pytest.approx(
                reference_row[2], 1e-10, 1e-3
            ), f"Failed for file {reference_file_name} row {i+1}."
