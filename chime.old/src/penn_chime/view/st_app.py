...
def main():
    ...
    if not m.reasonable_model_parameters:
        st.subheader(
            "Based on the current parameters, estimated pre-mitigation doubling time is greater than 15 days. Try different parameters."
        )
        return
    ...
    admits_chart = build_admits_chart(
        ...
        use_log_scale=p.use_log_scale,
    )

    st.altair_chart(admits_chart, use_container_width=True)
    display_download_link(
        st, p, filename=f"{p.current_date}_projected_admits.csv", df=m.admits_df,
    )
    ...
    display_download_link(
        st, p, filename=f"{p.current_date}_projected_census.csv", df=m.census_df,
    )
    ...
    st.markdown(
        i18n.t("app-PPE-documentation").format(
            ...
            link_to_tutorial="{docs_url}/ppe-calculator/ppe-basic-tutorial".format(
                docs_url=DOCS_URL
            ),
        ),
    ...
    display_download_link(
        st, p, filename=f"{p.current_date}_sim_sir_w_date.csv", df=m.sim_sir_w_date_df,
    )
    display_footer(st)
