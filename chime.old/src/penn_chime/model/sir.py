...
class Sir:
    def __init__(self, p: Parameters):
        ...
        if p.date_first_hospitalized is None and p.doubling_time is not None:
            ...
            if p.mitigation_date is None:
                ...
                self.i_day = i_day = int(
                    get_argmin_ds(raw["census_hospitalized"], p.current_hospitalized)
                )
                ...
                for self.i_day in range(p.n_days + PREFIT_ADDITIONAL_DAYS):
                    ...
                    loss = get_loss(
                        raw["census_hospitalized"][self.i_day], p.current_hospitalized
                    )
        ...
        self.raw["date"] = self.raw["day"].astype("timedelta64[D]") + np.datetime64(
            p.current_date
        )
