...
_SIDEBAR_ELEMENTS = ReadOnlyDict(
    OrderedDict(
        ###
...
class Sidebar(Page):
    ...
    input_value_map = ReadOnlyDict(
        OrderedDict(
            (
                key,
                {"number": "value", "date": "date", "switch": "on"}.get(value, "value"),
            )
            for key, value in input_type_map.items()
        )
    )

    input_state_map = ReadOnlyDict(
        OrderedDict(
            [
    ...
    def get_html(self) -> List[ComponentMeta]:
        ...
        """
        elements = [Store(id="sidebar-store")]
        ...
        sidebar = Nav(
            ...
            children=Div(children=elements, className="px-3 pb-5",),
            style={
                "bottom": 0,
                "left": 0,
                "overflowY": "scroll",
