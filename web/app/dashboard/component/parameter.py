...
def _build_component(meta):
    ...
    return html.Div(
        ...
        children=[
            html.Label(f"{label}", title=title, htmlFor=f"{component_id}"),
            dcc.Input(
                className="form-control", type=component_type, id=f"{component_id}"
            ),
        ],
    )
...
class ParameterInput(object):
    ...
    def __init__(self, component_meta):
        """Create the component from parameter metadata.

        :param component_meta: Dictionary containing the component metadata
         used to build the parameter input component.
        """
    ...
    @property
    def component(self):
        """Return the instantiated input component."""
...
class ParameterGroup(object):
    ...
    def __init__(self, parameter_group: Dict[str, str]) -> None:
        """Combine associated input components into component groups.

        :param parameter_group: Dictionary containing the metadata for
         multiple related input components.
        """

        self._label = html.Div(
            className="card-header",
            style={"textAlign": "center"},
            children=[
                html.H5(
                    f'{parameter_group["groupLabel"]}',
                    className="display-5",
    ...
    @property
    def container(self):
        """Return the component input group."""
        return html.Div(
            className="card",
            children=[
                self._label,
                html.Div(className="card-body", children=self._component_group),
