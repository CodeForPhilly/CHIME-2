...
# import numpy as np
# import pandas as pd
# from gvar import dump, gvar, mean, sdev
# from gvar._gvarcore import GVar  # pylint: disable=E0611
# from lsqfit import empbayes_fit, nonlinear_fit
...
#             lambda x: float(str(x).split("(")[1][:-1]) if "(" in str(x) else float(x)
#         )
...
#         loss_mcmc = (
#             np.mean((median_pred[:, 3] - test_set.hosp) ** 2)
#             + np.mean((median_pred[:, 5] - test_set.vent) ** 2)
#         ) / 2  # output
#
#         # prediction quantiles:  the proportion of times the prediction is greater than the mean
#
#         hq_m = [
#             (arrs_test[:, day, 3] > test_set.hosp.iloc[day]).mean()
#             for day in range(days_withheld)
#         ]
#         vq_m = [
#             (arrs_test[:, day, 5] > test_set.vent.iloc[day]).mean()
#             for day in range(days_withheld)
#         ]
#
...
#         hq_n = [
#             (
#                 np.random.normal(mmtail.hmu.iloc[day], mmtail.hsig.iloc[day], 10000)
#                 > test_set.hosp.iloc[day]
#             ).mean()
#             for day in range(days_withheld)
#         ]
#         vq_n = [
#             (
#                 np.random.normal(mmtail.vmu.iloc[day], mmtail.vsig.iloc[day], 10000)
#                 > test_set.vent.iloc[day]
#             ).mean()
#             for day in range(days_withheld)
#         ]
#
...
#
#         resh_n = mm.hmu[: len(data) + len(test_set)] - np.array(
#             data.hosp.tolist() + test_set.hosp.tolist()
#         )
#         resh_m = np.median(arrs[:, : len(data) + len(test_set), 3], axis=0) - np.array(
#             data.hosp.tolist() + test_set.hosp.tolist()
#         )
#         resv_n = mm.vmu[: len(data) + len(test_set)] - np.array(
#             data.vent.tolist() + test_set.vent.tolist()
#         )
#         resv_m = np.median(arrs[:, : len(data) + len(test_set), 5], axis=0) - np.array(
#             data.vent.tolist() + test_set.vent.tolist()
#         )
#
#         plotq = dict(
...
#         ax[i].plot(
#             csvout.loc[csvout.which_hospital == ho, "days_out"],
